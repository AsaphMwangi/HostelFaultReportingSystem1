...
public class StudentIssuesAdapter extends RecyclerView.Adapter<StudentIssuesAdapter.MyViewHolder> {
    Context context;
    ArrayList<Issues> IssuesArraylist;

    public StudentIssuesAdapter(Context context, ArrayList<Issues> issuesArraylist) {
    ...
    @NonNull
    @Override
    public StudentIssuesAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.single_issue,parent,false);
        return new MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull StudentIssuesAdapter.MyViewHolder holder, int position) {
        ...
        {
            Intent intent = new Intent(context,IssueDetails2.class);
